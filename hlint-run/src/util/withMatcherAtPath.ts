...
import type {ProblemMatcherDocument} from '../github';

const readFile = promisify(fs.readFile);

export async function addMatcherAtPath(
  matcherPath: string
): Promise<ProblemMatcherDocument> {
...
export async function removeMatcher(
  problemMatcherDocument: ProblemMatcherDocument
) {
...
export default async function withMatcherAtPath<T>(
  matcherPath: string,
  fn: () => Promise<T>
): Promise<T> {
