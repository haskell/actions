...
import {SEVERITY_LEVELS as HLINT_SEV_LEVELS} from './hlint';
import type {CheckMode, RunArgs} from './run';
...
function parseCheckMode(arg: string): CheckMode {
  arg = arg.toUpperCase();
  const matchingLevel = HLINT_SEV_LEVELS.find(
...
export default function getInputs(): RunArgs {
  const hlintCmd =
    core.getInput(INPUT_KEY_HLINT_BIN, {required: false}) || 'hlint';
