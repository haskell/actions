...
import type {Idea as HLintIdea, Severity as HLintSeverity} from './hlint';
import {
...
function getOverallCheckResult(
  failOn: CheckMode,
  {ideas, statusCode}: HLintResult
): CheckResult {
  const hintsBySev = HLINT_SEV_LEVELS.map((sev) => [
    sev,
...
export default async function run({
  ...
  const {ideas, statusCode} = await withMatcherAtPath(matcherDefPath, () =>
