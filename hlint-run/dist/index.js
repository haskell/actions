...
var MATCHER = new SingleLineMatcherFormat("hlint");
var MATCHER_DEF_PATH = path.join(".github", "hlint.json");
...
var INPUT_KEY_HLINT_BIN = "hlint-bin";
var INPUT_KEY_HLINT_FILES = "path";
...
async function addMatcherAtPath(matcherPath) {
  core2.debug(`Adding problem matcher at ${matcherPath}`);
  const fileContents = await readFile2(matcherPath, "utf8");
  const problemMatcherDocument = JSON.parse(fileContents);
  (0, import_command.issueCommand)("add-matcher", {}, matcherPath);
...
async function removeMatcher(problemMatcherDocument) {
  problemMatcherDocument.problemMatcher.forEach(({ owner }) => {
    (0, import_command.issueCommand)("remove-matcher", { owner }, "");
  });
}
async function withMatcherAtPath(matcherPath, fn) {
  const matcherDoc = await (async () => {
    try {
      ...
      core2.error(`Error adding problem matcher at path ${matcherPath}: ${e}`);
      return null;
    }
  })();
  try {
...
async function runHLint(cmd, args) {
  core3.info(`Running ${cmd} ${args.join(" ")}`);
  const { stdout: hlintOutputStr, statusCode } = await bufferedExec(cmd, args);
  core3.info(`hlint completed with status code ${statusCode}`);
...
function getOverallCheckResult(failOn, { ideas, statusCode }) {
  ...
  if (failOn === "STATUS" && statusCode !== 0) {
    ...
    const failedBySev = hintsBySev.slice(0, SEVERITY_LEVELS.indexOf(failOn) + 1).filter(([_sevName, numHints]) => numHints > 0);
...
var OUTPUT_KEY_HLINT_IDEAS = "ideas";
function setOutputs(result) {
  const { ok, ideas, statusCode, hintSummary } = result;
  core4.setOutput(OUTPUT_KEY_HLINT_IDEAS, ideas);
  if (ok) {
    if (hintSummary.length) {
      core4.info(`HLint finished with hints: ${hintSummary}`);
    } else {
      core4.info("HLint finished");
    }
  } else {
    core4.setFailed(`HLint failed with status: ${statusCode}. ${hintSummary}`);
  }
}
...
async function main() {
  try {
    ...
    const result = await core5.group("hlint", () => run(inputs));
    setOutputs(result);
  } catch (error2) {
    core5.setFailed(error2 instanceof Error ? error2 : String(error2));
  }
}
