...
async function ghcupBin(os: OS): Promise<string> {
  ...
  const bin = await tc.downloadTool(
    `https://downloads.haskell.org/ghcup/${ghcupVersion}/x86_64-${
      os === 'darwin' ? 'apple-darwin' : 'linux'
    }-ghcup-${ghcupVersion}`
  );
  await afs.chmod(bin, 0o755);
  return join(await tc.cacheFile(bin, 'ghcup', 'ghcup', ghcupVersion), 'ghcup');
}
...
export async function resetTool(
  ...
  switch (os) {
    ...
    case 'darwin':
      ...
      await exec(bin, ['unset', tool]);
      break;
    case 'win32': // We don't need to do anything here... yet
      break;
  }
}
